...
- **Privacy gate noise**: `privacy-block` no longer treats every unresolved shell expansion as sensitive, so `cat $FILE`, `wc -l *.cjs`, `cat ~/.zshrc` and `du -sh */` stop raising a prompt. A secret spelled with a tilde or a glob (`~/.ssh/id_rsa`, `certs/*.pem`) or handed to an unknown tool through a file-shaped option (`docker compose --env-file .env`) is still caught by name, and a quoted-delimiter heredoc is read as prose rather than shell. `RESTRICTED` no longer matches CafeKit's own source and docs by substring on `sessions`, `session-state`, `completion-authority` or `hook-log`. Pure indirection (`cat $FILE`) now goes through by design.
- **Codex privacy approval lifetime**: Codex honours no interactive `ask` from a hook, so an approval costs a full relay round trip. The request and grant window is now one hour instead of five minutes, the grant window starts at approval rather than inheriting the request deadline, a grant covers repeated access instead of being consumed on first use, it is no longer bound to the tool that tripped the gate, and a pasted approval prompt is trimmed. Grants stay bound to one session, cleared at session start, and matched to their exact path set.

### Added
- **Risk-proportional assurance routing**: normalized risk now raises the automatic assurance floor to `Elevated`, which keeps targeted adversarial inspection but does not invent independent-audit authority. `Strict` is opt-in only through an explicit user/project audit requirement or a user-confirmed scope-specific audit decision; risk keywords, severity labels, `Full` depth, and model preference never select it alone. An unavailable Strict host event pauses once with the exact blocker instead of causing reviewer retry loops, silent downgrade, or simulated attestation.
- **Domain-generic authoring quality**: every retention/lifecycle policy now requires clock anchor, clock source, timezone/precision, cutoff comparator/inclusivity, enforcement boundary, and a wrong-clock/boundary counterexample; every public, replay, or operator API contract now requires method, route, auth, headers, request schema, success response, error semantics, and idempotency or concurrency behavior; every executable implementation task now requires concrete test-file or artifact ownership or a shared proof boundary — a bare `npm test` string is not ownership. Specs-only records doc impact as a brief recommendation and never fabricates execution proof. Two-review target is advisory: after two failed semantic rounds, pause for unresolved product/security/architecture uncertainty or repeated non-convergence; one bounded mechanical correction without weakening gates is allowed; budgets never override correctness. Canonical `Criteria`/`Owner` V example and installed Claude/Codex `spec-readiness`/`--semantic-digest` commands are documented; authors may persist legitimate scope/decision/topology via canonical scaffold/sync paths while finalizer-owned `semantic_model`, `validation.semantic_review`, and readiness remain forbidden to hand-edit. Compact templates keep a single core anchor with proof conditional; self-tests assert per-surface contracts and parse the public V example, and Codex projection is verified via transform/temp fixture. Benchmark tuning targets are `≤10 min` for small tasks and `≤40 min, ≤500K tokens` for complex tasks with at most two review cycles — a tuning signal, not a reason to skip gates. `Direct` and `Compact` stay light and `spec.json` remains the sole machine authority.
