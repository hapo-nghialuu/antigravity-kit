...
function evidenceBody(text, policy) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    const match = lines[index].match(/^#{2,3}\s+(Evidence|Task Test Plan & Verification Evidence|Verification & Evidence)\b/);
    if (match) {
      start = index + 1;
      level = lines[index].match(/^#+/)[0].length;
      break;
    }
  }
  ...
  for (let index = start; index < lines.length; index += 1) {
    ...
    if (heading && heading[1].length <= level && !(policy.isTapMetadataHeading?.(lines[index]))) {
      end = index;
      break;
    }
  }
...
function receiptFailures(featureDir, taskPath, task, runtimeContext, policy) {
  const file = readTask(featureDir, taskPath);
  if (!file) return { failures: ['a'], body: null, file: null };
  const failures = [];
  const status = file.text.split('\n').find((line) => /^\s*(?:\*\*)?Status(?:\*\*)?\s*:/i.test(line));
  if (!status || !/\bdone\b/i.test(status)) failures.push('a');
  const body = evidenceBody(file.text, policy);
  if (!body || !/^\s*Verification:\s*PASS\s*$/m.test(body) || /\{\{[^}]+\}\}/.test(body)) {
    failures.push('c');
  } else {
    const options = policy.receiptValidatorOptions(task, { runtimeContext, requireProvenanceBinding: true });
    ...
    for (const failure of policy.validateCanonicalReceipt(body, options)) {
...
function resolveCandidate({ resolver, projectRoot, runtime, payload }) {
  ...
  const targeted = resolver.resolveActiveSpec({ projectRoot, runtime, explicitFeature, explicitPath });
...
function evaluateCloseout({ resolver, policy, projectRoot, runtime, payload }) {
  ...
  for (const [taskPath, task] of Object.entries(registry)) {
    ...
    const receipt = receiptFailures(featureDir, taskPath, task, runtimeContext, policy);
    if (receipt.failures.length) failures.push(`${taskPath}: checks ${receipt.failures.join(',')}`);
    if (receipt.body !== null) receiptBodies.set(taskPath, receipt.body);
    if (receipt.file) taskFiles.push({ path: taskPath, digest: cryptoDigest(receipt.file.bytes) });
  }
  ...
  const artifactFiles = artifactRefs.map((reference) => ({
    ...reference,
    digest: cryptoDigest(readBoundFile(projectRoot, reference.path) || Buffer.from('<missing>')),
  }));
  ...
  const binding = {
    ...
    spec_bytes_digest: cryptoDigest(specBytes || Buffer.from('<missing>')),
