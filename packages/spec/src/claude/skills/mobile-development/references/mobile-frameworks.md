...
```dart
testWidgets('Counter increments', (WidgetTester test-runner) async {
  await test-runner.pumpWidget(MyApp());

  expect(find.text('0'), findsOneWidget);
  expect(find.text('1'), findsNothing);

  await test-runner.tap(find.byIcon(Icons.add));
  await test-runner.pump();
