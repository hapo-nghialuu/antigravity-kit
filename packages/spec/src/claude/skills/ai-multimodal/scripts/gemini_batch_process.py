...
def find_api_key() -> Optional[str]:
    ...
    Uses ~/packages/spec/src/claude/scripts/resolve_env.py for consistent resolution across all skills.
    Falls back to local resolution if centralized resolver not available.

    Priority order (highest to lowest):
    1. process.env (runtime environment variables)
    2. PROJECT/packages/spec/src/claude/skills/ai-multimodal/.env (skill-specific)
    3. PROJECT/packages/spec/src/claude/skills/.env (shared skills)
    4. PROJECT/packages/spec/src/claude/.env (project global)
    5. ~/packages/spec/src/claude/skills/ai-multimodal/.env (user skill-specific)
    6. ~/packages/spec/src/claude/skills/.env (user shared)
    7. ~/packages/spec/src/claude/.env (user global)
    """
...
def get_default_model(task: str) -> str:
    ...
    if task == 'generate':  # Image generation
        ...
        if model:
            return model
        return 'gemma-4-31b-it'  # Existing default
...
def batch_process(
    ...
    if not api_key:
        ...
        print("  echo 'GEMINI_API_KEY=your-key' >> ~/packages/spec/src/claude/.env")
        print("\nOther options:")
        print("  - Run setup checker: python scripts/check_setup.py")
        print("  - Show full hierarchy: python ~/packages/spec/src/claude/scripts/resolve_env.py --show-hierarchy --skill ai-multimodal -v")
...
Examples:
  ...
  %(prog)s --files *.mp3 --task transcribe --model gemma-4-31b-it
  ...
  %(prog)s --task generate --prompt "A mountain landscape at sunset" \\
    --model gemma-4-31b-it-image --aspect-ratio 16:9 --size 2K
