...
SKILL_DIR=""
if [ -d "packages/spec/src/claude/skills/chrome-devtools/scripts" ]; then
  SKILL_DIR="packages/spec/src/claude/skills/chrome-devtools/scripts"
elif [ -d "$HOME/packages/spec/src/claude/skills/chrome-devtools/scripts" ]; then
  SKILL_DIR="$HOME/packages/spec/src/claude/skills/chrome-devtools/scripts"
fi
...
node "$SKILL_DIR/aria-snapshot.js" --url https://example.com --output ./packages/spec/src/claude/chrome-devtools/snapshots/page.yaml
```
...
mkdir -p packages/spec/src/claude/chrome-devtools/snapshots

# Capture and store with timestamp
SESSION="$(date +%Y%m%d-%H%M%S)"
node "$SKILL_DIR/aria-snapshot.js" --url https://example.com --output packages/spec/src/claude/chrome-devtools/snapshots/$SESSION.yaml
```
...
```bash
# Basic screenshot
node "$SKILL_DIR/screenshot.js" --url https://example.com --output ./packages/spec/src/claude/chrome-devtools/screenshots/page.png

# Full page
node "$SKILL_DIR/screenshot.js" --url https://example.com --output ./packages/spec/src/claude/chrome-devtools/screenshots/page.png --full-page true

# Specific element
node "$SKILL_DIR/screenshot.js" --url https://example.com --selector ".main-content" --output ./packages/spec/src/claude/chrome-devtools/screenshots/element.png
```
...
Store logs for analysis in `<project>/packages/spec/src/claude/chrome-devtools/logs/<session>/`:

```bash
# Create session directory
SESSION="$(date +%Y%m%d-%H%M%S)"
mkdir -p packages/spec/src/claude/chrome-devtools/logs/$SESSION

# Capture and store
node "$SKILL_DIR/console.js" --url https://example.com --duration 10000 > packages/spec/src/claude/chrome-devtools/logs/$SESSION/console.json
node "$SKILL_DIR/network.js" --url https://example.com > packages/spec/src/claude/chrome-devtools/logs/$SESSION/network.json

# View errors
jq '.messages[] | select(.type=="error")' packages/spec/src/claude/chrome-devtools/logs/$SESSION/console.json
```
