...
- If the task/spec is too vague to verify, use `/hapo:question` or ask one grounded clarification before planning.
...
2. **Plan** — choose the smallest coherent path; use `/hapo:question` when requirements are unclear and `/hapo:specs` for feature specs when ready.
