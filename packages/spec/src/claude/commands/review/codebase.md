...
* Use `/hapo:inspect ext` for scoped Gemini discovery or `/hapo:inspect` for scoped internal discovery to search the codebase for files needed to complete the task
...
### Plan
* Use `planner` subagent to analyze reports from `researcher` and `hapo:inspect` discovery runs to create an improvement plan following the progressive disclosure structure:
