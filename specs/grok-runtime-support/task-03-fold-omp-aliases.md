...
- `omp-hooks.test.js` keeps the contrast task 02 installed: the Claude hook answers `ask` and the omp hook answers `deny` on the same lowercase `bash` payload.
