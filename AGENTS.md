...
Primary operating instructions for Codex or any AI agent using this CafeKit runtime.
...
- Primary workflow: `./.Codex/rules/workflow.md`
- Development rules: `./.Codex/rules/ai-dev-rules.md`
- Skill workflow routing: `./.Codex/rules/skill-workflow-routing.md`
- Skill domain routing: `./.Codex/rules/skill-domain-routing.md`
- Subagent coordination: `./.Codex/rules/orchestrator.md`
- Docs maintenance: `./.Codex/rules/manage-docs.md`
- State sync: `./.Codex/rules/state-sync.md`
- Hook handling: `./.Codex/rules/hook-protocols.md`
- Other protocols: `./.Codex/rules/*`

## Skill And Script Use

- Evaluate the available skills catalog before work and activate the relevant skill(s).
- Use `./.Codex/rules/skill-workflow-routing.md` and `./.Codex/rules/skill-domain-routing.md` to choose skills. CafeKit does not auto-route prompts through a scoring hook.
- When needed, run `node .Codex/scripts/generate-skill-catalog.cjs --skills` to inspect the installed skill catalog.
- If there is a reasonable chance a skill applies, prefer the skill workflow over ad hoc execution.
- If modifying skills, edit the current project/runtime files, not `~/.Codex/skills` directly.
- Run Python skill scripts with the skill venv:
  - macOS/Linux: `.Codex/skills/.venv/bin/python3 scripts/<script>.py`
  - Windows: `.Codex\skills\.venv\Scripts\python.exe scripts\<script>.py`
- If a skill script fails, diagnose and fix the script or environment instead of abandoning the task.

## Git And Reporting

- Use conventional commits.
- Do not add AI attribution by default; if requested, add Codex credit as a footer/trailer, not in the subject.
...
Codex always addresses the user as "đại ca" throughout the conversation. If it stops doing so, it is a sign the context has been compacted/truncated — tell the user to consider `/clear`.
