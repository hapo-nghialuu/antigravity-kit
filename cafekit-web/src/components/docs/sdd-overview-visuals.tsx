...
const copy = {
  en: {
    ...
    stages: [
      ...
      ['Spec', 'Scope, requirements, design, and tasks become durable files.'],
    ...
    artifacts: [
      ['spec.json', 'Machine state: phase, scope lock, task registry, approvals.'],
      ['requirements.md', 'User-visible behavior and acceptance criteria.'],
      ['research.md', 'Source-backed findings and decisions.'],
      ['design.md', 'Architecture, contracts, data flow, invariants.'],
      ['tasks/task-R*.md', 'One implementation packet with files, steps, criteria, evidence.'],
    ],
    ...
    loop: [
      ...
      ['4. Sync state', 'Update spec.json and task markdown after proof.'],
    ],
    ...
    good: ['Repo files are the source of truth', 'Scope lock makes changes explicit', 'Missing details route back to the spec', 'Review compares code against the contract'],
  },
  vi: {
    eyebrow: 'Spec-driven development',
    ...
    stages: [
      ...
      ['Spec', 'Scope, requirements, design, và tasks thành file bền vững.'],
    ...
    artifacts: [
      ['spec.json', 'Machine state: phase, scope lock, task registry, approvals.'],
      ['requirements.md', 'Behavior user nhìn thấy và acceptance criteria.'],
      ['research.md', 'Finding và decision có source backing.'],
      ['design.md', 'Architecture, contracts, data flow, invariants.'],
      ['tasks/task-R*.md', 'Một gói implement gồm files, steps, criteria, evidence.'],
    ],
    ...
    loop: [
      ...
      ['4. Sync state', 'Update spec.json và task markdown sau khi có proof.'],
    ],
    ...
    good: ['File trong repo là source of truth', 'Scope lock bắt thay đổi phải explicit', 'Thiếu chi tiết thì quay lại spec', 'Review so code với contract'],
  },
  ja: {
    eyebrow: 'Spec-driven development',
    ...
    stages: [
      ['Request', '人が変更を依頼します。'],
      ['Spec', 'Scope、requirements、design、tasks が durable files になります。'],
    ...
    artifacts: [
      ['spec.json', 'Machine state: phase、scope lock、task registry、approvals。'],
      ['requirements.md', 'User-visible behavior と acceptance criteria。'],
      ['research.md', 'Source-backed findings と decisions。'],
      ['design.md', 'Architecture、contracts、data flow、invariants。'],
      ['tasks/task-R*.md', 'Files、steps、criteria、evidence を持つ implementation packet。'],
    ],
    ...
    loop: [
      ...
      ['4. Sync state', 'Proof 後に spec.json と task markdown を更新します。'],
    ],
    compareTitle: '普通の prompting との違い',
    bad: ['Chat が source of truth', 'Scope が黙って変わる', 'Assistant が不足を推測する', 'Review は final diff だけを見る'],
    good: ['Repo files が source of truth', 'Scope lock で変更が explicit', '不足は spec に戻す', 'Review は code と contract を比較する'],
  },
