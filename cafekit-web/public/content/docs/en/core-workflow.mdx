...
<DocsHero
  ...
  description="CafeKit's primary workflow is intentionally strict: clarify the request, write the spec, lock the task packet, implement one boundary, verify with real evidence, review adversarially, then sync state."
/>
...
```text
Idea
  -> /hapo:brainstorm        # if the request is still unclear
  ...
  -> /hapo:sync              # state and task docs stay aligned
```
...
## 1. Clarify before writing a spec
...
Brainstorming is scout-first. The assistant should inspect the relevant code or context before asking generic questions. This phase should not write implementation code. Its output should be a spec-ready decision set: what problem is being solved, what is out of scope, which architecture looks simplest, and which risks must be carried into the spec.

Skip this phase only when the request is already concrete enough to become requirements.

## 2. Turn the request into a spec
...
## 4. Implement one boundary at a time
...
## 5. Verify with evidence, not optimism
...
## 6. Review like the change is trying to break production
...
## 7. Sync state and documentation

Use `/hapo:sync <feature> <task> done` only after proof exists. CafeKit expects machine and human state to move together:
