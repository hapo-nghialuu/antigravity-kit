...
| Command | Use when |
|---|---|
| `/hapo:specs` | C1 scope、reviewed plan、flat executable tasks、C2 decision が必要。 |
...
| `/hapo:sync` | Observed task Status または final inline Receipt を proof の捏造なしで sync。 |
...
| `/hapo:fix` | diagnosis 後に小さく安全に bug を修正し regression checks を行う。 |
...
Codex CLI は同じ public workflow を native `$hapo-*` skills で提供します。
