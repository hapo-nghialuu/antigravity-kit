...
<DocsHero
  ...
  description="CafeKit の主 workflow は意図的に厳密です。request を明確にし、spec を作り、task packet を固定し、1つの boundary を実装し、実 evidence で verify し、adversarial review を通し、state を sync します。"
/>
...
```text
Idea
  -> /hapo:brainstorm        # request がまだ曖昧な場合
  ...
  -> /hapo:sync              # state と task docs を同期
```
...
Brainstorm は scout-first です。assistant は一般的な質問をする前に、関連 code や context を inspect するべきです。この phase では implementation code を書きません。出力は spec-ready な decision set です: 何を解くか、何を out of scope にするか、どの architecture が最も単純か、どの risk を spec に持ち込むか。

request が requirements に変換できるほど具体的な場合だけ、この phase を skip します。

## 2. Request を spec にする
...
## 5. Optimism ではなく evidence で verify する
...
## 6. Production を壊そうとしている change として review する
...
## 7. State と documentation を sync する

Proof が存在してから `/hapo:sync <feature> <task> done` を使います。CafeKit は machine state と human state が一緒に進むことを期待します:
