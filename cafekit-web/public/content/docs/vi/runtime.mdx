...
Claude Code dùng CommonJS hooks trong `.claude/hooks/`. OpenCode dùng TypeScript plugins trong `.opencode/plugins/` — xem [OpenCode platform](/docs/platforms/opencode) để biết mapping plugin ↔ hook.
...
- `spec.json` lưu phase, status, `task_files`, và `task_registry`.
- `tasks/task-R*.md` lưu context, steps, criteria, và evidence.
